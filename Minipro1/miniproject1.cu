 ...
 */
 ...
 __global__ void CalculateInvertedGrayScale( long int *r, long int *g, long int *b, long int *T1, int m, int n) {
     int id = blockIdx.x * blockDim.x + threadIdx.x;
     if (id < m * n) {
         int i = id / n;
         int j = id % n;
         int grey = (r[id] + g[id] + b[id]) / 3;
         T1[(m - 1 - i) * n + j] = grey;
     }
 }
 
 __global__ void CalculateThomasTransformation(long int *r, long int *g, long int *b, long int *T2, int m, int n) {
     int id = blockIdx.x * blockDim.x + threadIdx.x;
     if (id < m * n) {
         int thomas= floor(0.5 * r[id]) + floor(sqrt((double)g[id])) + b[id];
         T2[id] = thomas;
     }
 }
